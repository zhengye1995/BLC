...
def eval_map(det_results,
    ...
    Returns:
        tuple: (mAP, [dict, dict, ...])
    """
    print(len(det_results), len(gt_bboxes), len(gt_labels))
    ...
    if scale_ranges is not None:
        ...
        for cls_result in eval_results:
            if cls_result['num_gts'] > 0:
                aps.append(cls_result['ap'])
        mean_ap = np.array(aps).mean().item() if aps else 0.0
    print_zsd_summary(mean_ap, eval_results, dataset, area_ranges, num_classes=num_classes, class_wise=class_wise)
...
def print_map_summary(mean_ap, results, dataset=None, ranges=None):
    ...
    for i, cls_result in enumerate(results[:num_classes]):
    ...
    for i in range(num_scales):
        ...
        table_data.append(['reall@100', '', '', '', '', '{:.4f}'.format(recalls[i, :].mean())])
        table = AsciiTable(table_data)
        table.inner_footing_row_border = True
        print(table.table)


def print_zsd_summary(mean_ap, results, dataset=None, ranges=None, num_classes=17, class_wise=False):
    """Print mAP and results of each class.
    ...
    if class_wise:
        ...
        for i in range(num_scales):
            ...
            for j in range(num_classes):
                row_data = [
                    label_names[j],
                    '{:.1f}'.format(recalls[i, j]*100),
                    '{:.1f}'.format(aps[i, j]*100)
                ]
                table_data.append(row_data)
            table_data.append(['mAP', '', '{:.1f}'.format(mean_ap[i]*100)])
            table_data.append(['reall@100', '', '{:.2f}'.format(recalls[i, :].mean()*100)])
            table = AsciiTable(table_data)
...
def print_gzsd_summary(seen_mean_ap, unseen_mean_ap, results, dataset=None, ranges=None, num_classes=65, num_seen_classes=48, class_wise=False):
    """Print mAP and results of each class.
    ...
    recalls = np.zeros((num_scales, num_classes), dtype=np.float32)
    aps = np.zeros((num_scales, num_classes), dtype=np.float32)
    seen_recalls = np.zeros((num_scales, num_seen_classes), dtype=np.float32)
    seen_aps = np.zeros((num_scales, num_seen_classes), dtype=np.float32)
    unseen_recalls = np.zeros((num_scales, num_classes-num_seen_classes), dtype=np.float32)
    unseen_aps = np.zeros((num_scales, num_classes-num_seen_classes), dtype=np.float32)
    for i, cls_result in enumerate(results[:num_classes]):
        if cls_result['recall'].size > 0:
            recalls[:, i] = np.array(cls_result['recall'], ndmin=2)[:, -1]
        aps[:, i] = cls_result['ap']
    for i, cls_result in enumerate(results[:num_seen_classes]):
        if cls_result['recall'].size > 0:
            seen_recalls[:, i] = np.array(cls_result['recall'], ndmin=2)[:, -1]
        seen_aps[:, i] = cls_result['ap']
    ...
    if not isinstance(unseen_mean_ap, list):
        unseen_mean_ap = [unseen_mean_ap]
    if not isinstance(hm_ap, list):
        hm_ap = [hm_ap]
    if class_wise:
        if dataset is None:
            label_names = [str(i) for i in range(1, num_classes + 1)]
        ...
        for i in range(num_scales):
            ...
            for j in range(num_classes):
                ...
                table_data.append(row_data)
            table_data.append(['seen mAP', '', '{:.1f}'.format(seen_mean_ap[i] * 100)])
            table_data.append(['unseen mAP', '', '{:.1f}'.format(unseen_mean_ap[i] * 100)])
            table_data.append(['HM mAP', '', '{:.1f}'.format(hm_ap[i] * 100)])
            table_data.append(['seen reall@100', '', '{:.2f}'.format(seen_recalls[i, :].mean() * 100)])
            table_data.append(['unseen reall@100', '', '{:.2f}'.format(unseen_recalls[i, :].mean() * 100)])
            HM_recall = (2 * seen_recalls[i, :].mean() * 100 * unseen_recalls[i, :].mean() * 100) / \
                        (seen_recalls[i, :].mean() * 100 + unseen_recalls[i, :].mean() * 100)
            table_data.append(['HM reall@100', '', '{:.2f}'.format(HM_recall)])
            table = AsciiTable(table_data)
            table.inner_footing_row_border = False
            print(table.table)
    else:
