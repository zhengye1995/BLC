...
class AnchorSemanticHead(nn.Module):
    ...
    def __init__(self,
        ...
        super(AnchorSemanticHead, self).__init__()
        ...
        vec = vec_load[:, :self.num_classes]

        self.voc = torch.tensor(voc, dtype=torch.float32).cuda()
        self.vec = torch.tensor(vec, dtype=torch.float32)

        self.vec_bg_weight = self.vec[:, 0]
        self.vec_bg_weight = torch.tensor(self.vec_bg_weight).cuda().view([1, 300])

        self.kernel_semantic = nn.Linear(self.voc.shape[1], self.vec.shape[0])
        self.vec_bg = nn.Linear(300, 2, bias=False)
    ...
    def forward_single(self, x):
        ...
        cls_score = torch.mm(cls_score, self.voc)
        cls_score = self.kernel_semantic(cls_score)
        cls_score = self.vec_bg(cls_score)
